...
//sticky_box의 이미지를 바로 바꾸면 딱딱ㅎ하게 바껴서 살짝 줄였다가 다기 키우며 교체
...
//사진마다 살짝 다른 각도와 glitch 효과를 줘서 디자인 갤러리처럼 겹쳐 보이게
photos.forEach((photo, index) => {
  //사진이 등장하기 직전에 레이어 순서와 glitch 클래스를 조정
  pinTl.add(() => {
    //뒤에 나온 사진이 앞쪽에 쌓이도로 z-index 올림
...
//가로 스크롤 섹션에서 실제로 옆으로 움직여야되는 거리를 계산
//트랙 전체 너비에서 화면에 이미 보이는 너비를 빼면 이동 거리만 남음
function totalWidth() {
...
//horizontal_section은 세로 스크롤을 가로 이동으로 바꿔서 보여즘
const horizontalTween = gsap.to('.track', {
  ...
  scrollTrigger: {
    ...
    start: ' top top',
    //실제 이동 거리보다 더 길게 잡아서 앞뒤 여백 스크롤 만들기
...
//마지막 cta 버튼도 화면에  들어오면 자연스럽게 나타나게 
...
window.addEventListener('load',()=>{
  ...
  //document.fonts가 있는 브라우저라면 폰트 로딩도 기다림
  if(document.fonts){
    //폰트가 모두 준비된 뒤 scrolltrigger 위치를 다시 맞춘다
